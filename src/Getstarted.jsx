...
const Getstarted = () => {
  ...
  return (
    <>
       <div>
          <section className="get-started-body">
              <div className="get-started-left">
                <div className="dropdowns">
                  <section className="dropdown1 all-dropdown">
                    <button onClick={() => setOpen1(!open1)} className='dropdown-top-btn'>Ageny banking <span><img src={open1 ? Arrowup : Arrowdown} alt="" /></span> </button>
                    {open1 && (
                      <ul>
                      <li><button onClick={()=> {
                        return(
                          setdivleft("agencybanking"),
                          setOpen1(false)
                        )}}>What is Agency Banking</button></li>
                      <li><button  onClick={()=>{
                        return(
                          setdivleft("onboardagent"),
                          setOpen1(false)
                        )
                      }}>Onboarding Agents</button></li>
                      <li><button>Benefit with Kadick</button></li>
                    </ul>
                    )}
                  </section>
                  <section className="dropdown2 all-dropdown">
                    <button onClick={() => setOpen2(!open2)} className='dropdown-top-btn'>Registration Process<span><img src={Arrowdown} alt="" /></span> </button>
